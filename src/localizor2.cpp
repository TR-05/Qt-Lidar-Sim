...
Result localize2(std::pair<std::vector<float>, std::vector<float>> data, float heading) {
    ...
    for (float a = -45.0f; a <= 45.0f; a += 1.5f) {
        ...
        if (score > max_score) { max_score = score; best_offset = a; }
    }
    ...
    if (resL.found) curX = fabs(resL.position);
    else if (resR.found) curX = Config::RoomSize - resR.position;

    // Determine Y relative to Global Frame
    if (resB.found) curY = fabs(resB.position);
    else if (resT.found) curY = Config::RoomSize - resT.position;
