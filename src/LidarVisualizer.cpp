...
void LidarVisualizer::paintEvent(QPaintEvent* event) {
    ...
    for (const auto& obs : obstacles) {
        ...
        p.drawEllipse(toScreen(obs.x, obs.y), obs.radius * scale, obs.radius * scale);
    }
    ...
    */
    const std::vector<QColor> palette = {
        ...
        QColor(128, 128, 255, 150),  // Sky Blue
        QColor(255, 255, 128, 150),  // Yellow
        QColor(255, 128, 255, 150),  // Pink
        QColor(128, 255, 255, 150)   // Cyan
    };
    ...
    auto drawHistogram = [&](QPainter& painter, const std::vector<int>& hist, int xOffset, QString title) {
        if (hist.empty()) return;
        int chartWidth = hist.size() * barWidth;

        painter.setPen(QPen(QColor(60, 70, 90), 1));
        painter.setBrush(QColor(20, 25, 35));
        painter.drawRect(xOffset, chartBottom - chartHeight, chartWidth, chartHeight);
    ...
    drawHistogram(p, yHist, 30 + (280 * barWidth) + spacing, "Y-Axis Point Density");
    ...
    p.drawText(statusRect.adjusted(textMargin, 35 + lineSpacing, 0, 0), QString("Localized: X:%1 Y:%2").arg(estPose.x, 0, 'f', 2).arg(estPose.y, 0, 'f', 2));
    ...
    p.drawText(statusRect.adjusted(textMargin, 60 + (lineSpacing * 4), 0, 0), statusText);
...
void LidarVisualizer::drawRobot(QPainter& p, QPointF pos, float heading, bool isGhost, int size) {
    p.save();
    p.translate(pos);
    p.rotate(heading);
    float s = size;
...
void LidarVisualizer::mouseMoveEvent(QMouseEvent* event) {
    ...
    if (m_isDragging) {
        ...
        int padding = 7;
        QRectF boundary(padding, padding, scale * Config::RoomSize - 2 * padding, scale * Config::RoomSize - 2 * padding);
        // 3. Clamp the X and Y coordinates
        float clampedX = std::clamp((float)newPos.x(), (float)boundary.left(), (float)boundary.right());
        float clampedY = std::clamp((float)newPos.y(), (float)boundary.top(), (float)boundary.bottom());
        ...
        float boxWidth = 30;
        QRectF roomRect(m_objectPos.x() - boxWidth / 2, m_objectPos.y() - boxWidth / 2, boxWidth, boxWidth);
